...
import { SearchIcon, Loader2, Crown } from "lucide-react"
            ...
            <nav className="w-full px-4 flex items-center justify-between" aria-label="Global">
                <div className="flex items-center">
                    <div className="sm:hidden">
                        <Sheet>
                            <SheetTrigger className=' mt-2'><Menu /></SheetTrigger>
                    ...
                    <Button className="text-[14px] text-zinc-400 bg-secondary hover:bg-muted pr-2 hover:opacity-80" onClick={() => setOpen(true)}>
                        <Search className="mr-2 h-4 w-4 text-xs" />{navb('inputsearch')}
                                ...
                                <div>
                                    {searchResults.map((item) => (
                                        <Link href={`/media/${item.id}?mediaType=${item.media_type}`} key={item.id}>
                                            <div key={item.id} className="flex items-center space-x-4 px-4 py-2 hover:bg-secondary transition-colors cursor-pointer">
                                                {item.poster_path ? (
                                                    <Image
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="text-xs text-muted-foreground">
                                                        {item.media_type === 'movie' ? 'Movie' : 'TV Series'} • <b className="font-normal">
                ...
                {!isVip && ( // Condição para exibir o botão apenas se o usuário não for VIP
                        <Button className="bg-[#ffc34f]/20 mr-2 text-[#ffc34f] hover:bg-[#ffc34f]/30">
                            <Crown className="w-4 h-4 mr-1" />{navb('btnPremium')}
                        </Button>
                    )}
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="start">
                            <DropdownMenuItem onClick={() => signOut()} className="text-red-400 py-2">
                                <span><Icon.LogOut size={15} className="mr-2" /></span> {navb('btnlogout')}
